...
def tool_versions():
    ...
    for name, cmd in wanted.items():
        try:
            ...
            # Tools differ on whether they prefix their version with "v" — emu
            # reports "emu v3.6.2", chopper reports "chopper 0.13.0". Strip it
            # here so callers can add exactly one "v" without producing "vv".
            out[name] = line[0].split()[-1].removeprefix("v") if line else "unknown"
        except Exception:
            out[name] = "unknown"
    return out
...
def main():
    ...
    # Snakemake is invoked directly rather than via `nano16s`, so fall back to
    # the bare name rather than printing an empty version string.
    own_version = str(getattr(params, "version", "") or "").strip()
    nano16s_v = f" v{esc(own_version)}" if own_version else ""
    ...
    parts += [
        ...
        f"<p class='sub'>Sequence data were processed with nano16s{nano16s_v}. "
        f"Reads were merged per barcode, adapter-trimmed with "
        ...
        f"<footer>Generated by nano16s{nano16s_v}. This file is self-contained — "
